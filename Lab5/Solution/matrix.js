...
const showResult2D = (title, containerId, dataArray) => {
    ...
    for (let i = 0; i < dataArray.length; i++) {
        ...
        for (let j = 0; j < dataArray[i].length; j++) {
            let td = document.createElement('td');
            td.textContent = dataArray[i][j]; // Access the element directly from the 2D array
            tr.appendChild(td);
        }
...
// Please note that matrix data has been stored in a 1D array
// This shows that data structor can be different from presentation 
